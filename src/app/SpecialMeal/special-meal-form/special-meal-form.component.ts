...
MealName: any;
  ...
  constructor(
    ...
    private route: ActivatedRoute // Injecting ActivatedRoute to get route parameters
  ) {
  ...
  onEdit(): void {
    if (this.specialMeal.id) {
      ...
      this.specialMealService.updateSpecialMeal(this.specialMeal.id,updatedMeal).subscribe(
        response => {
